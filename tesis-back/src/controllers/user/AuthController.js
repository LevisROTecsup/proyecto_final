...
module.exports = {
  login: async (req, res) => {
    ...
    const token = JsonWebToken.createToken(tokenInfo, ["user:basic"], tokenDetailConstants);
  ...
  me: async (req, res) => {
    ...
    try {
      ...
      userData = await UserModel.findById(requestAdminId);
    ...
    return res.status(200).json({
      ...
      message: "Ok",
      data: {
        user: { code: userData._id, name: userData.name, email: userData.email, token },
